...
class Geometry {
  ...
  getMidPoint(ndxA: number, ndxB: number, cache: { [key: string]: number }): number {
...
function createProgram(gl: WebGL2RenderingContext, shaderSources: string[], transformFeedbackVaryings: string[] | null, attribLocations: { [key: string]: number } | null): WebGLProgram | null {
...
function makeVertexArray(gl: WebGL2RenderingContext, bufLocNumElmPairs: [WebGLBuffer, number, number][], indices: number[] | null): WebGLVertexArrayObject | null {
...
function makeBuffer(gl: WebGL2RenderingContext, sizeOrData: number | BufferSource, usage: number): WebGLBuffer | null {
...
class InfiniteGridMenu {
  ...
  constructor(canvas: HTMLCanvasElement, items: any[], onActiveItemChange: (index: number) => void, onMovementChange: (isMoving: boolean) => void, onInit: ((sketch: InfiniteGridMenu) => void) | null = null) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  private init(onInit: ((sketch: InfiniteGridMenu) => void) | null) {
    ...
    this.discVAO = makeVertexArray(
      gl,
      [
        [makeBuffer(gl, this.discBuffers.vertices, gl.STATIC_DRAW) as WebGLBuffer, this.discLocations.aModelPosition, 3],
        [makeBuffer(gl, this.discBuffers.uvs, gl.STATIC_DRAW) as WebGLBuffer, this.discLocations.aModelUvs, 2]
      ],
  ...
  private initTexture() {
    const gl = this.gl;
    this.tex = createAndSetupTexture(gl, gl.LINEAR, gl.LINEAR, gl.CLAMP_TO_EDGE, gl.CLAMP_TO_EDGE) as WebGLTexture;
    ...
    Promise.all(
      ...
      console.error("Error loading images for texture atlas:", err);
    });
  }
  ...
  private findNearestVertexIndex(): number | null {
...
export default function InfiniteMenu({ items = [] }: { items: Item[] }) {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      sketch = new InfiniteGridMenu(canvas, items.length ? items : defaultItems, handleActiveItem, setIsMoving, (sk: InfiniteGridMenu) =>
        sk.run()
      );
    }
